  ...
  async function test_initial_dump_is_loaded_as_synced_when_collection_is_empty() {
    ...
    for (let { client } of gBlocklistClients) {
      ...
      ok(
        ...
        `Loaded dump of ${client.collectionName} has timestamp ${timestamp}`
      );
    }
  }
);
...
add_task(
  async function test_initial_dump_is_loaded_when_using_get_on_empty_collection() {
    for (let { client } of gBlocklistClients) {
      if (
        IS_ANDROID &&
        client.collectionName !=
      ...
      ok(
        ...
        `Loaded dump of ${client.collectionName} has ${
          afterLoaded.length
        } records`
      );
    }
...
add_task(async function test_sync_event_data_is_filtered_for_target() {
  ...
  for (let { client } of gBlocklistClients) {
    ...
    equal(
      called,
      false,
      `shouldn't have sync event for ${client.collectionName}`
    );
    ...
    equal(
      ...
      1,
      `event filtered data for ${client.collectionName}`
    );
    ...
    ok(
      internalData.length > current.length,
      `event current data for ${client.collectionName}`
    );
  }
});
add_task(clear_state);
...
add_task(async function test_inspect_with_blocklist_clients() {
  ...
  deepEqual(collections[0], {
    ...defaults,
    collection: "gfx",
    serverTimestamp: 3000,
  });
  deepEqual(collections[1], {
    ...defaults,
    collection: "addons",
    serverTimestamp: 3000,
  });
  deepEqual(collections[2], {
    ...defaults,
    collection: "plugins",
    serverTimestamp: 3000,
  });
  ...
  for (const c of inspected.collections) {
    ...
    ok(
      ...
      `${c.collectionName} last check ${c.lastCheck}`
    );
  }
});
add_task(clear_state);
...
add_task(async function test_bucketname_changes_when_bucket_pref_changes() {
  ...
  Services.prefs.setCharPref(
    "services.blocklist.addons.bucket",
    "addons-preview"
  );
  ...
  equal(
    BlocklistGlobal.ExtensionBlocklistRS._client.bucketName,
    "addons-preview"
  );
