                         ...
                         AutoIdVector* props)
{
    ...
    for (size_t n = 0; n < properties.length(); n++) {
        ...
        if (!Enumerate<true>(cx, obj, id, enumerable, flags, ht, props))
            return false;
    }
...
{
    if (checkForDuplicates)
        return EnumerateNativeProperties<true>(cx, pobj, flags, ht, props);
    return EnumerateNativeProperties<false>(cx, pobj, flags, ht, props);
}
...
Snapshot(JSContext* cx, HandleObject pobj_, unsigned flags, AutoIdVector* props)
{
    ...
    do {
        if (pobj->getClass()->getNewEnumerate()) {
            ...
            if (pobj->isNative()) {
                if (!EnumerateNativeProperties(cx, pobj.as<NativeObject>(), flags, ht, props, true)) {
                    return false;
                }
            }
...
JSCompartment::getOrCreateIterResultTemplateObject(JSContext* cx)
{
    ...
    RootedObjectGroup group(
      cx, ObjectGroupCompartment::makeGroup(cx, templateObject->getClass(), proto));
    if (!group)
        return iterResultTemplate_; // = nullptr
    templateObject->setGroup(group);

    // Set dummy `value` property
    if (!NativeDefineProperty(cx,
                              templateObject,
                              cx->names().value,
