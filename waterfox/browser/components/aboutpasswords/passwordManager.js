...
async function _Startup() {
  ...
  filterField.addEventListener("input", FilterPasswords);
...
const signonsTreeView = {
  ...
  async setCellText(row, col, value) {
    ...
    if (col.id === "userCol") {
      await editLogin("username");
    } else if (col.id === "passwordCol") {
      if (!value) {
        return;
      }
      await editLogin("password");
    }
  },
};
...
async function getUserFacingLogins() {
  const logins = await Services.logins.getAllLogins();
  return logins.filter(login => {
    login.QueryInterface(Ci.nsILoginMetaInfo);
    return lazy.LoginHelper.isUserFacingLogin(login);
  });
}

async function LoadSignons() {
  try {
    signons = await getUserFacingLogins();
  } catch (e) {
...
async function FinalizeSignonDeletions(syncNeeded) {
  for (const login of deletedSignons) {
...
function FocusFilterBox() {
  if (document.activeElement !== filterField) {
    filterField.focus();
  }
}
