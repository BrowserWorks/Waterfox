...
account-setup-looking-up-db = Recherche de la configuration : base de données des FAI de Waterfox…
...
# Note: Do not translate or replace Waterfox. It stands for the public project mozilla.org, not Waterfox Corporation. The database is a generic, public domain facility usable by any client.
account-setup-success-settings-db = Configuration trouvée dans la base de données des FAI de Waterfox.
