...
# Waterfox -main  （Waterfoxの固有の設定名称）
...
pinned-icon-only =
    .label = ピン留めタブを縮小してサイトのアイコンのみを表示
insert-after-current =
    .label = 新しいタブを開いた際に、新しいタブを現在表示しているタブの後ろに開く
insert-related-after-current =
    .label = 現在のタブの後ろに、新しい関連したタブを開く
dynamic-theme-header = { -brand-short-name } のシステムテーマ・{ -brand-short-name } Lepton のカラー設定
dynamic-theme-dark =
    .label = ダークモードを強制する
dynamic-theme-light =
    .label = ライトモードを強制する
dynamic-theme-auto =
    .label = システムに追従する
restart-header = パネル UI の再起動ボタンの設定
restart-show-button =
    .label = ブラウザーを再起動することのできるボタンをポップアップメニューに表示します
restart-purge-cache =
    .label = ブラウザ再起動時に起動時キャッシュ（高速起動のための一時的なファイル）をクリアする
restart-require-confirmation =
    .label = 再起動時に確認する
tab-feature-header = タブのコンテキストメニュー
show-duplicate-tab =
    .label = 重複しているタブのメニュー項目を表示する
show-copy-url =
    .label = コピーしたタブの URL メニュー項目を表示する
enable-copy-active-tab =
    .label = アクティブなタブのみ URL をコピーする
show-copy-all-urls =
    .label = すべてのタブの URL をコピーするメニュー項目を表示する
show-unload-tab =
    .label = タブのコンテキストメニューにタブをアンロードするボタンを表示する   
statusbar-header = ステータスバーの設定
statusbar-enabled =
    .label = ステータスバーを表示
...
geolocation-api-header = ジオロケーション API（位置情報 API）
geolocation-description = 一部のウェブサイトでは、ユーザーの位置情報が必要な場合があります。位置情報が見つからず、ウェブサイトが機能しない場合は、この設定を有効にしてから再度お試しください。
...
send-referrer-header-1 =
    .label = リンクや類似の要素をクリックしたときのみ、リファラーヘッダーを送信する。
send-referrer-header-2 =
    .label = すべてのリクエストでリファラーヘッダーを送信する (デフォルト)
enable-webrtc-p2p =
    .label = WebRTC ピア接続の有効化
load-images =
    .label = 画像を自動的に読み込む
enable-javascript =
    .label = JavaScript を有効にする（無効にすると、ほとんどのウェブサイトが動作しません）
webrtc-header = WebRTCピア接続
ref-header = HTTPリファラーヘッダー
