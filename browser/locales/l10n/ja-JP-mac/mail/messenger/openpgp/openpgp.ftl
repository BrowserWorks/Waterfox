...
fail-cancel = エラー - ユーザーによって鍵サーバーからの鍵の読み込みが中止されました
not-first-block = エラー - 最初の OpenPGP ブロックは公開鍵ではありません
import-key-confirm = メッセージ中に埋め込まれた鍵を読み込みますますか？
fail-key-import = エラー - 鍵の読み込みに失敗しました
file-write-failed = ファイル { $output } への書き出しに失敗しました
no-pgp-block = エラー - 有効な OpenPGP データブロックが見つかりません
confirm-permissive-import = 読み込みに失敗しました。読み込みを試みた鍵は破損しているか、未知の属性が使われています。正常な部分のみの読み込みを試みますか？この場合、読み込みが完了しない、あるいは利用できない鍵となる可能性があります。
...
key-in-message-body = メッセージ中に鍵が含まれています。“鍵を読み込む” をクリックして鍵を読み込んでください
sig-mismatch = エラー - 署名が一致しません
invalid-email = エラー - 無効なメールアドレスです
attachment-pgp-key =
    開こうとしている添付ファイル '{ $name }' は OpenPGP 鍵ファイルです。“読み込み” をクリックして鍵を読み込むか、“表示” をクリックしてファイルの内容をウインドウに表示してください
