...
function WaterfoxClassicProfileMigrator() {
  ...
  if (AppConstants.platform == "macosx") {
    waterfoxUserDataFolder = FileUtils.getDir("ULibDir", ["Application Support", "Waterfox"], false);
  } else if (AppConstants.platform == "linux") {
    waterfoxUserDataFolder = FileUtils.getDir("Home", ["waterfox"], false);
  } else if (AppConstants.platform == "win") {
    waterfoxUserDataFolder = FileUtils.getDir("AppData", ["Waterfox"], false);
  }
...
WaterfoxClassicProfileMigrator.prototype._getAllProfiles = function() {
  ...
  if (-1 == profileName.search("68-edition")){ // Make sure it's not a current profile
    let folderProfile = [];
    let sourceFolder = null;
    if (AppConstants.platform == "macosx") {
        if (rootFolder == "") {
        folderProfile = ["Application Support", "Waterfox", profileName];
        } else {
        folderProfile = ["Application Support", "Waterfox", rootFolder, profileName];
        }
        sourceFolder = FileUtils.getDir("ULibDir", folderProfile, false);
    } else if (AppConstants.platform == "linux") {
        if (rootFolder == "") {
        folderProfile = ["waterfox", profileName];
        } else {
        folderProfile = ["waterfox", rootFolder, profileName];
        }
    ...
    Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype._getAllProfiles: Invalid profile" + profileName);
  }    
  return allProfiles;
...
WaterfoxClassicProfileMigrator.prototype.getSourceProfiles = function() {
  Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype.getSourceProfiles: ");
  return [...this._getAllProfiles().keys()]
    .map(x => ({ id: x, name: x }))
    .sort(sorter);
};

WaterfoxClassicProfileMigrator.prototype._getFileObject = function(dir, fileName) {
  Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype._getFileObject: " + fileName);
  let file = dir.clone();
...
WaterfoxClassicProfileMigrator.prototype.getResources = function(aProfile) {
  ...
  Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype.getResources: " + aProfile);
  ...
  Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype.getResources: " + sourceProfileDir + " : " + currentProfileDir);
    ...
    return {
      ...
      migrate(aCallback) {
        for (let file of files) {
          ...
          Services.console.logStringMessage("WaterfoxClassicProfileMigrator.prototype._getResourcesInternal: " + file.path + "\\" + file.leafName + " " + currentProfileDir.path  + "\\" + currentProfileDir.leafName);
