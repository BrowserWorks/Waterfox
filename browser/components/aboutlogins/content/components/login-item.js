...
class LoginItem extends ReflectedFluentElement {
  ...
  connectedCallback() {
    ...
    this.attachShadow({ mode: "open" }).appendChild(
      loginItemTemplate.content.cloneNode(true)
    );
    ...
    let copyUsernameButton = this.shadowRoot.querySelector(
      ".copy-username-button"
    );
    let copyPasswordButton = this.shadowRoot.querySelector(
      ".copy-password-button"
    );
    copyUsernameButton.relatedInput = this.shadowRoot.querySelector(
      "modal-input[name='username']"
    );
    copyPasswordButton.relatedInput = this.shadowRoot.querySelector(
  ...
  handleSpecialCaseFluentString(attrName) {
    switch (attrName) {
      ...
      case "copy-password-button": {
        ...
        let newAttrName =
          attrName.substr(0, attrName.indexOf("-")) + "-button-text";
      ...
      case "copy-username-button": {
        ...
        let newAttrName =
          attrName.substr(0, attrName.indexOf("-")) + "-button-text";
        copyUsernameButton.setAttribute(
          newAttrName,
          this.getAttribute(attrName)
        );
        break;
      ...
      case "modal-input-reveal-checkbox-hide": {
        this.shadowRoot
          .querySelector("modal-input[name='password']")
  ...
  render() {
    ...
    let hostnameNoScheme =
      this._login.hostname && new URL(this._login.hostname).hostname;
    this.shadowRoot.querySelector(".title").textContent =
      hostnameNoScheme || "";
    this.shadowRoot.querySelector(".hostname").textContent =
      this._login.hostname || "";
    this.shadowRoot
      .querySelector("modal-input[name='username']")
      .setAttribute("value", this._login.username || "");
    this.shadowRoot
      .querySelector("modal-input[name='password']")
      .setAttribute("value", this._login.password || "");
  }
  ...
  toggleEditing(force) {
    let shouldEdit =
      force !== undefined ? force : !this.hasAttribute("editing");
